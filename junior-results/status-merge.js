...
(function mergeJuniorAthleteStatus() {
  ...
  data.results = data.results.map(original => {
    ...
    // Enforce staff-approval gate. Do not let a legacy or inferred dual flag
    // affect qualification unless staff approval is explicit.
    ...
    if (record) {
      ...
      if (record.dualDeclared && !row.dualOtherCountry) {
        appendUnique(reviews, 'Dual citizen requires staff approval before affecting qualification');
      }
      if (record.approval && record.approval !== 'N/A' && !record.approval.startsWith('Approved')) {
        appendUnique(reviews, record.approval);
      }
    ...
    if (preMergeDualOtherCountry && !row.dualOtherCountry) {
      appendUnique(reviews, 'Legacy dualOtherCountry suppressed pending staff approval');
    }
