...
const S = {
  ...
  zoom: {k:1, x:0, y:0},
  painting: false,
  detailRegion: null,   // region index for zip drill-down
  scenarioId: null,
  scenarioName: '',
  dirty: false,
  booted: false,
...
function computeTallies(){
  const y = S.year;
  const rows = S.regions.map(()=>({m:0,a:0,c:0,cl:new Set(),zips:0,counties:0}));
  ...
  for (const [fips, st] of Object.entries(S.geo.stats)){
    ...
    const tgt = (ri==null || ri<0 || ri>=rows.length) ? un : rows[ri];
  ...
  // counties assigned (including empty ones)
  const cCount = S.regions.map(()=>0);
  Object.values(S.assign).forEach(ri => { if (ri>=0 && ri<cCount.length) cCount[ri]++; });
  rows.forEach((r,i)=>r.countiesAssigned = cCount[i]);
  return {rows, un};
}

function regionZips(ri){
  const y = S.year;
  const out = [];
  for (const [fips, st] of Object.entries(S.geo.stats)){
    if (S.assign[fips] !== ri) continue;
...
function renderPanel(){
  ...
  const rowsHtml = t.rows.map((r,i)=>`
    <tr class="${S.detailRegion===i?'sel':''}" data-drill="${i}">
      <td><span class="sw" style="background:${S.regions[i].color}"></span><b>${esc(S.regions[i].name)}</b></td>
  ...
  let drill = '';
  if (S.detailRegion!=null && S.detailRegion>=0 && S.detailRegion<S.regions.length){
    const zips = regionZips(S.detailRegion);
    const r = S.regions[S.detailRegion];
    drill = `<div class="bs-drill">
      <div class="bs-drill-h"><span class="sw" style="background:${r.color}"></span><b>${esc(r.name)}</b> — ${fmt(zips.length)} zip codes pooled (${yLabel}) <button class="tab" id="bsDrillClose" style="padding:3px 10px;font-size:11px">close</button></div>
...
function wirePanel(){
  ...
  bind('bsNew', ()=>{
    ...
    S.assign={}; S.regions=defaultRegions(12); S.active=0; S.scenarioId=null; S.scenarioName=''; S.detailRegion=null; S.dirty=false;
...
async function saveScenario(){
  ...
  const data = JSON.stringify({regions:S.regions, assign:S.assign, year:S.year, v:1});
  try {
    await NEON.query(
