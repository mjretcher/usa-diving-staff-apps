...
import { engine, json } from './runtime.js';
...
export async function compute2025Model(options = {}) {
  ...
  S.hostMin = 0;
  let recaps = null;
  try { recaps = await json('recaps-2025.json'); } catch (e) {}
  ...
  const perTier = Object.keys(tiers).sort((a, b) => a - b).map((L) => {
    ...
    return {
      ...
      // 2025 advancement also admits anyone meeting the average bar, so place
      // counts are not a cap. Rule places are in structure.rules instead.
      spots: null,
      fillRate: null,
  ...
  return {
    ...
    structure: {
      levels: [{ name: 'Regionals', meets: 12 }, { name: 'Zones', meets: 6 }, { name: 'Junior Nationals', meets: 1 }],
      rules: [
        { from: 'Regionals', text: 'Top 15 in each springboard event, plus anyone below 15th who meets the 15th-place average (±1 SD) bar, advance to Zones. Platform is exhibition at Regionals.', places: 12 * 15 * 16, placesText: '12 meets × 15 × 16 springboard events' },
        { from: 'Zones', text: 'Places 1–3 go to the Junior Nationals semifinal; springboard 4–10 and platform 4–7 go to the preliminaries.', places: 6 * (10 * 16 + 7 * 8), placesText: '6 meets × (10 × 16 springboard + 7 × 8 platform events)' },
      ],
    },
