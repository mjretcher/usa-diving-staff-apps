...
# NOTE (2026-07-28): this file previously contained the whole of main() TWICE, each
# with its own __main__ guard, so every run executed both in sequence — double the
# DiveMeets fetches and double the writes. The two copies had diverged: the second
# (which ran last and therefore won) hardcoded round 1 in the DiveSheets URL, silently
# reverting the round-aware fix kept above. That broke any event whose entries are not
# in round 1 — a single-round Qualifier lives in round 9, and so do the Junior Nationals
# synchro events (e.g. /DiveSheets/12923/30665/9). Keep exactly ONE main() and one guard.
