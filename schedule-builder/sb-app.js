...
function renderTlBar(timed){
  ...
  return`<div class="tl-bar ${lk?'is-locked':''}">
    ...
    ${typeof liveOn==='function'?`<button class="tl-textbtn ${liveOn()?'on':''}" onclick="liveToggle()" title="${liveOn()?'The run sheet is on. Every session is showing Start, Finish and Edit times. Click to turn it off and show planned times only.':'Turn on the run sheet to record what actually happens \u2014 tap Start when a session begins, or type the time in by hand afterwards.'}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3.5" fill="currentColor" stroke="none"/></svg> ${liveOn()?'Run sheet on':'Run sheet'}</button>`:''}
