(function () {
  ...
  function openPracticeTimelineLabel(event) {
    ...
    if (!note || /^(open practice block|full-day open practice|open practice|open training|break|awards|custom block)\.?$/i.test(note)) return title;
    return `${title} - ${note}`;
  }
  ...
  function renderScheduleBlockPlanner() {
    ...
    const types = ["Open Practice", "Restricted Training", "Open Training", "Break", "Awards", "Custom Block"];
  ...
  window.actions = {
    ...
    updateScheduleBlockPlanner(field, value) {
      ...
      if (field === "start" || field === "end") {
        ...
        if (!scheduleBlockPlanner.note || /^(Open practice\.|Open training\.|Break\.|Awards\.|Custom schedule note\.)$/i.test(scheduleBlockPlanner.note)) {
          scheduleBlockPlanner.note = defaultNotes[value] || value;
        }
      } else {
    ...
    createScheduleBlockFromPlanner() {
      ...
      update((draft) => {
        ...
        block.title = planner.type || "Open Practice";
    ...
    addScheduleBlock(dayId, afterSessionId = "", position = "end") {
      ...
      const defaultNote = title === "Open Practice" ? "Open practice." : title === "Open Training" ? "Open training." : title;
